...
import type { GuidebookActionState } from "@/features/guidebooks/actions";
...
export function GuidebookForm({ action }: GuidebookFormProps) {
  ...
  const [submissionState, submissionAction, isSubmitting] = useActionState(
    async (_previous: GuidebookActionState, formData: FormData) => action(formData),
    initialState,
  );

  useEffect(() => {
    if (submissionState.success) {
      router.push("/admin/resources/guidebooks");
      router.refresh();
    }
  ...
  return (
    <Card className="border-slate-200 bg-white py-0 shadow-sm">
      <CardHeader className="border-b border-slate-200 py-6">
        <CardTitle className="text-xl text-slate-900">가이드북 정보</CardTitle>
      </CardHeader>
        ...
        <form action={submissionAction} className="space-y-5">
          ...
          <div className="flex items-center gap-2">
            ...
            <Button
              variant="outline"
              type="button"
              onClick={() => router.push("/admin/resources/guidebooks")}
            >
              취소
            </Button>
          </div>
